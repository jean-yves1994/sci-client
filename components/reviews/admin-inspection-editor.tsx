...
export function AdminInspectionEditor({ inspectionId }: { inspectionId: string }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setOwner(d.owner ? { fullName: d.owner.fullName ?? '', phone: d.owner.phone ?? '', email: d.owner.email ?? '', occupancyStatus: d.owner.occupancyStatus ?? '', ownershipType: d.owner.ownershipType ?? '' } : { fullName: '', phone: '', email: '', occupancyStatus: '', ownershipType: '' });
  ...
  const saveValues = async () => {
    ...
    try {
      ...
      await api.patch(`/inspections/${inspectionId}/admin-edit/values`, { values: payload, baseVersion: inspection.version });
  ...
  const saveOwner = async () => {
    ...
    setBusy(true); setError(null);
    try {
      await api.patch(`/inspections/${inspectionId}/admin-edit/owner`, { ...owner, baseVersion: inspection.version });
      await load(); setNotice('Owner information updated.');
    } catch (e) { if (e instanceof ApiError && e.code === 'INSPECTION_STALE_VERSION') { setError(`${e.message} Reloaded the latest inspection.`); await load(); } else setError(readableError(e)); }
    finally { setBusy(false); }
  };

  return <Card className="border-primary/30"><CardHeader title="Administrative inspection edit" description="Administrator-only correction of the inspector's recorded data. Changes are version-checked and audited."/><div className="space-y-6 px-5 pb-5">
