...
export default function InspectionDetailPage() {
  ...
  const canEditReview = canDecide || (can('reviews.decide') && inspection.reviewer?.id === user?.id && awaitingDecision);
  const canGenerateReport = can('reports.generate') && ['APPROVED', 'REPORT_GENERATED'].includes(inspection.status);
  const draftReports = [...inspection.reports]
    .filter((report) => report.reportNumber.startsWith('DRF-'))
    .sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime());
  const officialReports = [...inspection.reports]
    .filter((report) => report.reportNumber.startsWith('RPT-'))
    .sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime());
  ...
  const downloadFinalReport = async () => {
    ...
    setBusy(true); setError(null); setNotice(null);
    try {
      // A final report is authoritative only when it was generated after the
      // inspection's approval/reviewer changes. Older reports can remain in
      // history, so regenerate once when the selected final predates approval.
      const reportIsOlderThanApproval = Boolean(
        inspection.approvedAt && new Date(latestReport.generatedAt).getTime() < new Date(inspection.approvedAt).getTime(),
      );
      if (reportIsOlderThanApproval) {
        const refreshed = await api.post<{ id: string }>(`/inspections/${inspection.id}/report`);
        await load();
        await downloadReport(refreshed.id);
        setNotice('The latest final report was regenerated from the professional review data.');
      } else {
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      <Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-5"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-semibold tracking-tight text-ink">{inspection.inspectionNumber}</h1><StatusBadge tone={statusTone(inspection.status)}>{statusLabel(inspection.status)}</StatusBadge><Badge tone={priorityTone(inspection.priority)}>{humanise(inspection.priority)}</Badge>{inspection.submissionCount > 1 && <Badge tone="neutral">Submission {inspection.submissionCount}</Badge>}</div><p className="mt-1.5 text-sm text-ink-muted">{inspection.loanReference} · {inspection.property.reference} · {addressValue}</p><dl className="mt-4 flex flex-wrap gap-x-8 gap-y-2.5"><HeaderFact label="Branch" value={inspection.branch.code} /><HeaderFact label="Inspector" value={fullName(inspection.inspector)} /><HeaderFact label="Reviewer" value={fullName(inspection.reviewer)} /><HeaderFact label="Due" value={formatDate(inspection.dueDate)} /></dl></div><div className="flex flex-wrap gap-2">{canEditReview && <Button variant="primary" onClick={() => router.push(`/inspections/${inspection.id}/review`)}>Edit inspection</Button>}{latestDraft && <Button variant="secondary" icon={<IconDownload className="h-4 w-4" />} onClick={() => void downloadReport(latestDraft.id)}>Download draft</Button>}{canGenerateReport && <Button variant="secondary" loading={busy} onClick={() => void run(() => api.post(`/inspections/${inspection.id}/report`), 'Report generated.')}>{latestReport ? 'Regenerate' : 'Generate report'}</Button>}{latestReport && <Button loading={busy} icon={<IconDownload className="h-4 w-4" />} onClick={() => void downloadFinalReport()}>Download final</Button>}</div></div></Card>
      ...
      <div className="space-y-4"><Card><CardHeader title="Location verification" /><div className="space-y-3.5 px-5 pb-5">{inspection.proximity ? <><div className="flex items-start gap-3"><span className={cx('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl', {success:'bg-success-bg text-success-fg',warning:'bg-warning-bg text-warning-fg',danger:'bg-danger-bg text-danger-fg',neutral:'bg-neutral-bg text-neutral-fg'}[PROXIMITY_TONE[inspection.proximity.verdict] as 'success' | 'warning' | 'danger' | 'neutral'])}><IconMapPin /></span><div className="min-w-0"><Badge tone={PROXIMITY_TONE[inspection.proximity.verdict]}>{humanise(inspection.proximity.verdict)}</Badge><p className="mt-1.5 text-xs leading-relaxed text-ink-muted">{inspection.proximity.explanation}</p></div></div>{inspection.locations[0] && <><dl className="space-y-2 border-t border-line pt-3.5 text-sm"><Detail label="Coordinates" value={`${Number(inspection.locations[0].latitude).toFixed(5)}, ${Number(inspection.locations[0].longitude).toFixed(5)}`} /><Detail label="Accuracy" value={inspection.locations[0].accuracyM ? `${Math.round(inspection.locations[0].accuracyM)} m` : '—'} /><Detail label="Captured" value={formatDateTime(inspection.locations[0].capturedAt)} /></dl>{inspection.locations[0].isMocked && <Alert tone="warning" title="Mock location reported">The device reported a simulated position. Recorded for your judgement, not treated as proof of wrongdoing.</Alert>}<a href={`https://www.google.com/maps/search/?api=1&query=${inspection.locations[0].latitude},${inspection.locations[0].longitude}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded text-xs font-medium text-brand-600 hover:text-brand-700">Open in Google Maps →</a></>} </> : <p className="text-sm text-ink-faint">No GPS reading captured.</p>}</div></Card>
        <Card><CardHeader title="Valuation" />{inspection.valuation ? <div className="px-5 pb-5"><p className="text-2xs font-medium text-ink-muted">Market value</p><p className="mt-1 text-2xl font-semibold tracking-tight text-ink">{formatMoney(inspection.valuation.marketValue, inspection.valuation.currency)}</p><dl className="mt-3.5 space-y-2.5 border-t border-line pt-3.5 text-sm"><Detail label="Forced sale" value={formatMoney(inspection.valuation.forcedSaleValue, inspection.valuation.currency)} /><Detail label="Replacement" value={formatMoney(inspection.valuation.replacementCost, inspection.valuation.currency)} /><Detail label="Rental estimate" value={formatMoney(inspection.valuation.rentalEstimate, inspection.valuation.currency)} /></dl>{inspection.valuation.comments && <p className="mt-3.5 border-t border-line pt-3.5 text-sm text-ink-muted">{inspection.valuation.comments}</p>}</div> : <p className="px-5 pb-5 text-sm text-ink-faint">Not yet recorded.</p>}</Card>
        <Card><CardHeader title="History" /><ol className="px-5 pb-5">{inspection.statusEvents.map((event, index) => <li key={event.id} className="flex gap-3"><div className="flex flex-col items-center"><span className={cx('mt-1.5 h-2 w-2 shrink-0 rounded-full ring-4', {success:'bg-success-solid ring-success-bg',warning:'bg-warning-solid ring-warning-bg',danger:'bg-danger-solid ring-danger-bg',info:'bg-info-solid ring-info-bg',accent:'bg-accent-solid ring-accent-bg',neutral:'bg-neutral-solid ring-neutral-bg',brand:'bg-brand-500 ring-brand-50'}[statusTone(event.toStatus)])} />{index < inspection.statusEvents.length - 1 && <span className="my-1 w-px flex-1 bg-line" />}</div><div className="min-w-0 pb-4"><p className="text-sm font-medium text-ink">{statusLabel(event.toStatus)}</p><p className="mt-0.5 text-2xs text-ink-faint">{fullName(event.actor)} · {formatDateTime(event.createdAt)}</p>{event.comment && <p className="mt-1.5 rounded-lg bg-surface-2 px-3 py-2 text-xs text-ink-muted">{event.comment}</p>}</div></li>)}</ol></Card>
        {inspection.comments.length > 0 && <Card><CardHeader title="Comments" /><div className="space-y-3.5 px-5 pb-5">{inspection.comments.map((comment) => <div key={comment.id} className="flex gap-2.5"><Avatar size="sm" name={initials(comment.author)} tone={avatarTone(comment.author.id)} /><div className="min-w-0"><p className="text-sm text-ink">{comment.body}</p><p className="mt-0.5 text-2xs text-ink-faint">{fullName(comment.author)} · {formatDateTime(comment.createdAt)}</p></div></div>)}</div></Card>}
        {inspection.reports.length > 0 && <Card><CardHeader title="Reports" description="Draft reports are available for review before approval; final reports are created after approval." /><div className="space-y-1.5 px-5 pb-5">{[...inspection.reports].sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime()).map((report) => { const isDraft = report.reportNumber.startsWith('DRF-'); return <div key={report.id} className="flex items-center justify-between gap-3 rounded-xl bg-surface-2 px-4 py-3"><div className="flex min-w-0 items-center gap-2.5"><Badge tone={isDraft ? 'warning' : 'success'}>{isDraft ? 'Draft' : 'Final'}</Badge><div className="min-w-0"><p className="truncate text-sm font-medium text-ink">{report.reportNumber}</p><p className="text-2xs text-ink-faint">v{report.version} · {formatDate(report.generatedAt)}</p></div></div><Button size="sm" variant="secondary" loading={busy && !isDraft} onClick={() => void (isDraft ? downloadReport(report.id) : downloadFinalReport())}>Download</Button></div>; })}</div></Card>}
      </div></div>

      <Modal open={decision !== null} onClose={() => { setDecision(null); setReason(''); }} title={decision === 'approve' ? 'Approve inspection' : decision === 'reject' ? 'Reject inspection' : 'Request corrections'} description={decision === 'approve' ? 'The official report will be generated and the inspector notified.' : 'The inspector will be notified with your reason.'} footer={<><Button variant="secondary" onClick={() => { setDecision(null); setReason(''); }}>Cancel</Button><Button variant={decision === 'reject' ? 'danger' : decision === 'approve' ? 'success' : 'primary'} loading={busy} disabled={decision !== 'approve' && !reason.trim()} onClick={() => { const base = { baseVersion: inspection.version }; if (decision === 'approve') void run(() => api.post(`/inspections/${inspection.id}/approve`, { ...base, note: reason.trim() || undefined }), 'Inspection approved. The report is being generated.'); else if (decision === 'reject') void run(() => api.post(`/inspections/${inspection.id}/reject`, { ...base, reason: reason.trim() }), 'Inspection rejected.'); else void run(() => api.post(`/inspections/${inspection.id}/request-correction`, { ...base, reason: reason.trim() }), 'Returned to the inspector for correction.'); }}>{decision === 'approve' ? 'Approve' : decision === 'reject' ? 'Reject' : 'Request corrections'}</Button></>}><div className="pb-2"><Field label={decision === 'approve' ? 'Note (optional)' : 'Reason'} required={decision !== 'approve'} hint={decision === 'correction' ? 'Be specific; the inspector sees this verbatim.' : undefined}><Textarea rows={4} value={reason} onChange={(e) => setReason(e.target.value)} placeholder={decision === 'correction' ? 'e.g. Please provide a clear photograph of the rear elevation.' : decision === 'reject' ? 'Explain why this inspection cannot be accepted.' : 'Any note to record with the approval.'} /></Field></div></Modal>
      {lightbox && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-6 backdrop-blur-sm animate-fade-in" onClick={() => setLightbox(null)} role="dialog" aria-label="Photograph"><img src={lightbox} alt="Inspection photograph" className="max-h-full max-w-full rounded-2xl" /></div>}
    </div>
  );
}

function HeaderFact({ label, value }: { label: string; value: string }) { return <div><dt className="text-2xs font-medium text-ink-faint">{label}</dt><dd className="mt-0.5 text-sm font-medium text-ink">{value}</dd></div>; }
function Detail({ label, value }: { label: string; value: string | number | null }) { return <div className="flex justify-between gap-4"><dt className="shrink-0 text-ink-muted">{label}</dt><dd className="truncate text-right font-medium text-ink">{value ?? '—'}</dd></div>; }
function conditionTone(condition: string): Tone { const tones: Record<string, Tone> = { EXCELLENT:'success', GOOD:'success', FAIR:'warning', POOR:'danger', CRITICAL:'danger' }; return tones[condition] ?? 'neutral'; }
function RatingDots({ rating }: { rating: number | null }) { if (rating === null) return <span className="text-2xs text-ink-faint">Not rated</span>; return <span className="flex items-center gap-1" title={`${rating} out of 5`}><span className="sr-only">{rating} out of 5</span>{[1,2,3,4,5].map((value) => <span key={value} className={cx('h-1.5 w-1.5 rounded-full', value <= rating ? 'bg-brand-500' : 'bg-surface-3')} aria-hidden />)}</span>; }
