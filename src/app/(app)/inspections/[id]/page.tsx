...
export default function InspectionDetailPage() {
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      <Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-5"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="text-xl font-semibold tracking-tight text-ink">{inspection.inspectionNumber}</h1><StatusBadge tone={statusTone(inspection.status)}>{statusLabel(inspection.status)}</StatusBadge><Badge tone={priorityTone(inspection.priority)}>{humanise(inspection.priority)}</Badge>{inspection.submissionCount > 1 && <Badge tone="neutral">Submission {inspection.submissionCount}</Badge>}</div><p className="mt-1.5 text-sm text-ink-muted">{inspection.loanReference} · {inspection.property.reference} · {addressValue}</p><dl className="mt-4 flex flex-wrap gap-x-8 gap-y-2.5"><HeaderFact label="Branch" value={inspection.branch.code} /><HeaderFact label="Inspector" value={fullName(inspection.inspector)} /><HeaderFact label="Reviewer" value={fullName(inspection.reviewer)} /><HeaderFact label="Due" value={formatDate(inspection.dueDate)} /></dl></div><div className="flex flex-wrap gap-2">{latestDraft && <Button variant="secondary" icon={<IconDownload className="h-4 w-4" />} onClick={() => void downloadReport(latestDraft.id)}>Download draft</Button>}{canGenerateReport && <Button variant="secondary" loading={busy} onClick={() => void run(() => api.post(`/inspections/${inspection.id}/report`), 'Report generated.')}>{latestReport ? 'Regenerate' : 'Generate report'}</Button>}{latestReport && <Button icon={<IconDownload className="h-4 w-4" />} onClick={() => void downloadReport(latestReport.id)}>Download final</Button>}</div></div></Card>
