...
// Re-exported so existing `import type { Tone } from '@/components/ui'`
// sites keep working. The definition now lives in lib/tone.ts.
import type { Tone } from '../lib/tone';
export type { Tone };

/** Tone → token class pairs. One place, so a status never drifts between screens. */
...
export const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  function Input({ className, ...rest }, ref) {
    return <input ref={ref} className={cx(control, 'h-10 px-3.5', className)} {...rest} />;
  },
);
...
export function Textarea({ className, ...rest }: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea className={cx(control, 'px-3.5 py-2.5', className)} {...rest} />;
}

export function Select({ className, children, ...rest }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <div className="relative">
      <select className={cx(control, 'h-10 appearance-none px-3.5 pr-9', className)} {...rest}>
        {children}
      </select>
      <svg
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={controlId} className="block text-xs font-medium text-ink-muted">
        {label}
        {required && <span className="ml-0.5 text-danger-fg" aria-hidden>*</span>}
...
export function Card({
  className, children, interactive, as: Tag = 'div',
}: {
  className?: string;
...
export function CardHeader({
  ...
  title: React.ReactNode; description?: string; action?: React.ReactNode; className?: string;
}) {
  return (
    <div className={cx('flex items-start justify-between gap-4 px-5 pb-4 pt-5', className)}>
...
export function Badge({
  ...
  return (
    ...
    >
      {dot && <span className={cx('h-1.5 w-1.5 rounded-full', TONE_DOT[tone])} aria-hidden />}
      {children}
    </span>
  );
}
...
/**
 * Status badge. Always renders the label as text as well as colour, so the
...
export function Alert({
  tone = 'danger', title, children, action, onDismiss,
}: {
  tone?: Tone; title: string; children?: React.ReactNode;
  action?: React.ReactNode; onDismiss?: () => void;
}) {
  return (
    <div className={cx('rounded-xl px-4 py-3', TONE_SURFACE[tone])} role="alert">
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="flex items-center gap-4 px-5 py-3.5">
          {Array.from({ length: columns }).map((__, colIndex) => (
            <Skeleton
              key={colIndex}
              className={cx('h-3.5', colIndex === 0 ? 'w-28' : colIndex === 1 ? 'flex-1' : 'w-16')}
            />
          ))}
        </div>
      ))}
...
export function Spinner({ className, label = 'Loading' }: { className?: string; label?: string }) {
  return (
    <div className={cx('flex items-center justify-center py-20', className)} role="status">
...
export function StatCard({
  label, value, icon, tone = 'brand', hint, change, sparkline, href,
}: {
  label: string;
  value: string | number;
  icon?: React.ReactNode;
  tone?: Tone;
  hint?: string;
  /** Omitted entirely when there is no honest comparison to make. */
  ...
  const body = (
    <>
      <div className="flex items-start justify-between gap-3">
        <span className={cx('flex h-9 w-9 items-center justify-center rounded-xl', TONE_SURFACE[tone])}>
          {icon}
        </span>
        {change && (
          <span
            className={cx(
              ...
              direction === 'up' ? 'bg-success-bg text-success-fg'
                : direction === 'down' ? 'bg-danger-bg text-danger-fg'
      ...
      {(hint || change) && (
        <p className="mt-2 truncate text-2xs text-ink-faint">{change?.label ?? hint}</p>
      )}
    </>
  );
  ...
  return href ? (
    <a href={href} className={cx(className, 'hover:shadow-lift')}>{body}</a>
  ) : (
...
export function ChartCard({
  title, description, action, children, loading, empty, footer,
}: {
  title: string;
  description?: string;
...
export function Th({
  children, className, onSort, sorted, align = 'left',
}: {
  children?: React.ReactNode;
  className?: string;
  onSort?: () => void;
  ...
  const content = (
    <span className={cx('inline-flex items-center gap-1', align === 'right' && 'flex-row-reverse')}>
      {children}
      {onSort && (
        <span className={cx('text-[8px]', sorted ? 'text-brand-500' : 'text-ink-faint/60')} aria-hidden>
...
export function Tr({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) {
  return (
    <tr
      className={cx('transition-colors duration-100 hover:bg-surface-2', onClick && 'cursor-pointer')}
      onClick={onClick}
    >
      {children}
    </tr>
...
export function Modal({
  ...
  React.useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // Focus is trapped inside the dialog: tabbing to the page behind an open
      // modal leaves keyboard users somewhere they cannot see.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="scroll-slim max-h-[60vh] overflow-y-auto px-5">{children}</div>
        {footer && (
...
export function Dropdown({
  trigger, children, align = 'right', width = 'w-56',
}: {
...
export function MenuItem({
  onClick, icon, active, danger, children,
}: {
  onClick?: () => void; icon?: React.ReactNode; active?: boolean;
  ...
  return (
    <button
      ...
      className={cx(
        ...
        danger ? 'text-danger-fg hover:bg-danger-bg'
          : active ? 'bg-surface-3 text-ink font-medium'
    ...
    >
      ...
      {active && (
        <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 shrink-0 text-brand-500" fill="none" stroke="currentColor" strokeWidth="2.2" aria-hidden>
...
export function Avatar({
  name, size = 'md', tone = 'neutral',
}: {
  name: string; size?: 'sm' | 'md' | 'lg'; tone?: Tone;
}) {
  const sizes = { sm: 'h-6 w-6 text-[10px]', md: 'h-8 w-8 text-xs', lg: 'h-10 w-10 text-sm' };
  return (
    <span
      className={cx('inline-flex shrink-0 items-center justify-center rounded-full font-semibold', sizes[size], TONE_SURFACE[tone])}
      aria-hidden
    >
      {name}
    </span>
...
export function Segmented<T extends string>({
  ...
  return (
    <div className="inline-flex rounded-xl border border-line bg-surface-2 p-0.5" role="tablist" aria-label={label}>
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
...
export function PageHeader({
  title, description, action, breadcrumb,
}: {
  title: string; description?: string; action?: React.ReactNode; breadcrumb?: React.ReactNode;
}) {
  return (
...
export function ProgressBar({ percentage, tone }: { percentage: number; tone?: Tone }) {
  const colour =
    tone ? TONE_DOT[tone]
      : percentage >= 100 ? 'bg-success-solid'
  ...
  return (
    ...
    >
      <div
        className={cx('h-full rounded-full transition-all duration-500 ease-swift', colour)}
...
/** Tooltip for the collapsed sidebar; CSS-only so it costs nothing to render. */
